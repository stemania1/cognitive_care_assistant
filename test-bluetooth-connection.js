 ...
 * This script helps diagnose ESP32 Bluetooth connection issues
 * Run: node test-bluetooth-connection.js [COM_PORT]
 */
...
console.log('');
console.log('========================================');
console.log('ESP32 Bluetooth Connection Test');
console.log('========================================');
console.log('');
...
async function testConnection() {
  ...
  try {
    ...
    if (ports.length === 0) {
      ...
      console.log('   2. Connect ESP32 via USB or pair via Bluetooth');
    ...
    if (TEST_PORT) {
      ...
      console.log('🔌 Step 2: Testing all ports for ESP32 data...');
      console.log('');
      console.log('💡 This will try to connect to each port for 5 seconds');
      console.log('   Make sure your ESP32 is powered on and paired!');
      console.log('');
      ...
      if (!foundPort) {
        console.log('');
        console.log('❌ Could not find ESP32 on any port.');
        console.log('');
        console.log('💡 Troubleshooting:');
        console.log('   1. Power on your ESP32');
        console.log('   2. Pair in Windows: Settings → Bluetooth & devices');
        console.log('   3. Look for "MyoWare_EMG" and pair it');
        console.log('   4. Make sure it shows as "Connected" (green checkmark)');
        console.log('   5. Check Device Manager → Ports (COM & LPT)');
        console.log('   6. Try: node test-bluetooth-connection.js COM8 (use your COM port)');
      } else {
        console.log('');
        console.log(`✅ SUCCESS! ESP32 found on ${foundPort}`);
        console.log('');
        console.log(`💡 To connect, run:`);
        console.log(`   node bluetooth-receiver.js ${foundPort}`);
      }
    }
    ...
    port.on('open', () => {
      ...
      timeout = setTimeout(() => {
        if (!receivedData && !errorOccurred) {
          ...
          console.log(`   (This port might not be the ESP32)`);
          console.log('');
      ...
      if (!receivedData) {
        ...
        try {
          ...
          if (data.type === 'emg_data' || data.muscleActivity !== undefined) {
            console.log(`   🎯 THIS IS YOUR ESP32!`);
            console.log(`   Data type: ${data.type || 'emg_data'}`);
            console.log(`   Muscle activity: ${data.muscleActivity || 'N/A'}`);
          }
        } catch (e) {
